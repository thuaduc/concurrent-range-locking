...
SongRangeLock::SongRangeLock(u64 no_threads)
...
auto SongRangeLock::AllocNode(u64 start, u64 end, u8 level) -> SkipListNode * {
 ...
 */
void SongRangeLock::FindNodes(u64 key, SkipListNode **out_nodes) {
  SkipListNode *q;
  auto p = root_;
...
void SongRangeLock::InsertRange(SkipListNode **nodes, u64 start, u64 end) {
...
auto SongRangeLock::TryLockRange(u64 start, u64 len) -> bool {
  Ensure(start > 0);
...
auto SongRangeLock::TestRange(u64 start, u64 len) -> bool {
  Ensure(start > 0);
  auto end = start + len - 1;
  SkipListNode *nodes[MAX_LEVEL];
...
void SongRangeLock::UnlockRange(u64 start) {
  Ensure(start > 0);
  SkipListNode *q;
